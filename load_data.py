...
def t2p_loader(batch_size, input_dict):
    train_src_path = os.path.join('ПУТЬ ДО ФАЙЛА train_names.pkl')
    train_trg_path = os.path.join('ПУТЬ ДО ФАЙЛА train_palettes_rgb.pkl')
    val_src_path = os.path.join('ПУТЬ ДО ФАЙЛА test_names.pkl')
    val_trg_path = os.path.join('ПУТЬ ДО ФАЙЛАtest_palettes_rgb.pkl')
...
def test_loader(dataset, batch_size, input_dict):
    ...
    if dataset == 'bird256':
        ...
        txt_path = 'ПУТЬ ДО ФАЙЛА hexcolor_vf/test_names.pkl'
        pal_path = 'ПУТЬ ДО ФАЙЛА test_palettes_rgb.pkl'
        img_path = 'ПУТЬ ДО ФАЙЛА test_images_origin.txt' # кажется вообще не нужно
